...
```
lsblk ********************************** sn genel sıradan halini görmek için 

df -h formatlanmış olan kullanılabilir diski gösterir net hali hazırda ki durum 
```
...
```
sudo file -s /dev/xvdf       formatı varmı harddiskin? sonuç data ise formatsız**************
```
- if not formatted, format the new volume
```
sudo mkfs -t ext4 /dev/xvdf    *** format atar ext4 tipinde dev altında xvdf yi formatla****
```
- check the format of the volume again after formatting
```
sudo file -s /dev/xvdf    *****tekrar format kontrol data alınmamalı*****
```
- create a mounting point path for new volume (volume-1)********m. point oluşturma

mount etmeden df -h sonuç vermez format var ama heln erişilemez
```
sudo mkdir /mnt/mp1     ------device altına bağlamak amaçlı kapı açılır----
```
- mount the new volume to the mounting point path
```
sudo mount /dev/xvdf /mnt/mp1/     xvdf yi mnt kapısı altında mp1 e mounth eder
```
